...
def mock_backends():
    """バックエンドのモック"""
    with patch("src.api.query_api.FaissBackend") as mock_faiss, \
        ...
        faiss_instance.search = AsyncMock(return_value=[
...
class TestHealthCheck:
    ...
    def test_health_check_degraded(self, client):
        """バックエンドに問題がある場合"""
        with patch.object(app.state.FB, "health_check", new_callable=AsyncMock) as mock_faiss_health:
